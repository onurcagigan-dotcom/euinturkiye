...
export default function IlanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    getDataProvider().getSavedListings(firma.id).then((saved) => {
      setIsSaved(saved.some((s) => s.listingId === id));
    });
  }, [firma, id]);
  ...
  const toggleSave = async () => {
    ...
    if (isSaved) {
      ...
      const sv = all.find((s) => s.listingId === id);
      if (sv) await getDataProvider().removeSavedListing(sv.id);
      setIsSaved(false);
    } else {
      await getDataProvider().saveListing_bookmark({
        id: `sv-${Date.now()}`, subscriberId: firma.id, listingId: id, savedAt: new Date().toISOString(),
      });
      setIsSaved(true);
    }
  };
    ...
    <PageShell>
      <div className="max-w-2xl mx-auto px-6 py-8">
        ...
        <div className="flex items-start justify-between gap-3 mt-4">
          <h1 className="text-2xl md:text-3xl font-bold text-ink leading-tight flex-1">{listing.title}</h1>
          {firma && (
            <button onClick={toggleSave}
              className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-semibold border transition-colors ${isSaved ? "bg-eu-pale border-eu text-eu" : "border-line text-slate hover:border-eu hover:text-eu"}`}>
              {isSaved ? "✓ İzleniyor" : "+ İzle"}
            </button>
          )}
        </div>
        {listing.subject && (
