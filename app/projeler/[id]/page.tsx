      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        {(project.ownerSubscriberName || (project.consortiumMembers && project.consortiumMembers.length > 0)) && (
          <div className="mb-10 bg-white border border-line rounded-2xl p-5">
            <h2 className="text-sm font-bold text-mist uppercase tracking-wide mb-3">Konsorsiyum</h2>
            <div className="flex flex-wrap gap-2">
              {project.ownerSubscriberName && (
                <span className="inline-flex items-center gap-1.5 text-sm bg-eu text-white px-3 py-1.5 rounded-full font-medium">
                  🏆 {project.ownerSubscriberName}
                  <span className="text-xs text-blue-200">Yürütücü</span>
                </span>
              )}
        ...
        <div className="mt-12">
          {isOwner ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-6 flex items-start gap-4">
              ...
              <div>
                <h3 className="font-bold text-ink mb-1">Bu projenin yürütücüsüsünüz</h3>
                <p className="text-slate text-sm">Konsorsiyum üyelik taleplerini <Link href="/firma" className="text-eu font-semibold hover:underline">firma panelinizden</Link> yönetebilirsiniz.</p>
              </div>
            </div>
          ) : isMember ? (
            <div className="bg-eu-pale border border-eu/20 rounded-xl p-6 flex items-start gap-4">
              <div className="text-3xl">🤝</div>
              <div>
                <h3 className="font-bold text-ink mb-1">Bu projenin konsorsiyum üyesisiniz</h3>
                <p className="text-slate text-sm">Yürütücü: {project.ownerSubscriberName}</p>
              </div>
            </div>
          ) : firma ? (
            myRequest ? (
              <div className="bg-white border border-line rounded-xl p-6 flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-bold text-ink mb-1">
                    {myRequest.status === "bekliyor" ? "Talebiniz inceleniyor" : myRequest.status === "onaylandi" ? "Talebiniz onaylandı" : "Talebiniz reddedildi"}
                  </h3>
                  <p className="text-slate text-sm">
                    {myRequest.requestedRole === "yurutucu" ? "Yürütücü" : "Konsorsiyum üyesi"} olarak başvurdunuz.
                    {" "}{myRequest.approverType === "admin" ? "Admin onayında." : "Yürütücü firma onayında."}
                  </p>
                </div>
              </div>
            ) : showRequestForm ? (
              <div className="bg-eu-pale border border-eu/20 rounded-xl p-6">
                <h3 className="font-bold text-ink mb-1">{firma.organization ?? firma.name} olarak başvuru</h3>
                <p className="text-slate text-sm mb-4">
                  {hasOwner
                    ? <>Bu projenin yürütücüsü <strong>{project.ownerSubscriberName}</strong>. Konsorsiyum üyesi olmak için başvurunuz onlara gidecek.</>
                    : "Bu projenin henüz bir yürütücüsü yok. Talebiniz admin onayına gidecek."}
                </p>

                {!hasOwner && (
                  <div className="mb-4">
                    <label className="block text-sm font-semibold text-ink mb-2">Hangi rolü talep ediyorsunuz?</label>
                ...
                <div className="mb-4">
                  <label className="block text-sm font-semibold text-ink mb-1">Açıklama (opsiyonel)</label>
                  <textarea value={requestNote} onChange={(e) => setRequestNote(e.target.value)}
                    rows={3} placeholder="Projedeki rolünüzü kısaca açıklayın…"
              ...
              <div className="bg-eu-pale border border-eu/20 rounded-xl p-6 flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate text-sm mb-3">
                    {hasOwner
                      ? <>Bu projenin yürütücüsü <strong>{project.ownerSubscriberName}</strong>. Konsorsiyum üyesi olmak için başvurabilirsiniz.</>
                      : "Bu projenin henüz bir yürütücüsü yok. Yürütücü veya konsorsiyum üyesi olarak başvurabilirsiniz."}
                  </p>
